...
def get_equities_dataset(assets=("SPX", "DJI"), with_vol=True):
    ...
    if assets == ("SPX",):
        df_asset = oxford[oxford["Symbol"] == ".SPX"].set_index(
            ["Unnamed: 0"]
        )  # [start:end]
        ...
        data_raw = np.concatenate(
            [rtn_spx, vol_spx[:, 1:], rtn_dji, vol_dji[:, 1:]], axis=-1
        )
    else:
...
def get_var_dataset(window_size, batch_size=5000, dim=3, phi=0.8, sigma=0.5):
    ...
    def get_pipeline():
        transforms = list()
        transforms.append(
            ("standard_scale", StandardScalerTS(axis=(0, 1)))
        )  # standard scale
...
def get_arch_dataset(window_size, lag=4, bt=0.055, N=5000, dim=1):
    ...
    def get_raw_data(N=5000, lag=4, T=2000, omega=0.00001, bt=0.055, burn_in=2000):
        ...
        for t in range(lag - 1, T + burn_in - 1):
            arch[:, t + 1] = omega + np.matmul(
                beta.reshape(1, -1),
                np.square(logrtn[:, t - lag + 1 : t + 1]).transpose(),
            )  # * (logrtn[:, t] < 0.)
...
def get_mit_arrythmia_dataset(filenames):
    ...
    for fn in filenames:
        records.append(
            wfdb.rdsamp(os.path.join(DATA_DIR, fn), sampto=3000)[0][None, ...]
        )
...
def get_etth_dataset(dataset_name):
    path = "./data/iTransformer_datasets/ETT-small"
    df = pd.read_csv(os.path.join(path, f"{dataset_name}.csv"), index_col=0)
    if dataset_name == "ETTh1.csv" or dataset_name == "ETTh2.csv":
        df = df[: 12 * 30 * 24]  # CHANGED: informer train split
    elif dataset_name == "ETTm1.csv" or dataset_name == "ETTm2.csv":
        df = df[: 12 * 30 * 24 * 4]
...
def get_custom_dataset(dataset_name):  # added
    path = f"./data/iTransformer_datasets/{dataset_name}"
    df = pd.read_csv(os.path.join(path, f"{dataset_name}.csv"), index_col=0)
...
def get_data(data_type, p, q, **data_params):
    if data_type == "VAR":
        ...
        pipeline, x_real_raw, x_real = get_etth_dataset("ETTh1")
    elif data_type == "etth2":
        pipeline, x_real_raw, x_real = get_etth_dataset("ETTh2")
    elif data_type == "ettm1":
        pipeline, x_real_raw, x_real = get_etth_dataset("ETTm1")
    elif data_type == "ettm2":
        pipeline, x_real_raw, x_real = get_etth_dataset("ETTm2")
    elif data_type == "ILI":
        pipeline, x_real_raw, x_real = get_custom_dataset("ILI")
    elif data_type == "us_births":
        pipeline, x_real_raw, x_real = get_custom_dataset("us_births")
    elif data_type == "weather":
        pipeline, x_real_raw, x_real = get_custom_dataset("weather")  # added
    elif data_type == "traffic":
        pipeline, x_real_raw, x_real = get_custom_dataset("traffic")
    elif data_type == "electricity":
        pipeline, x_real_raw, x_real = get_custom_dataset("electricity")
    elif data_type == "exchange_rate":
        pipeline, x_real_raw, x_real = get_custom_dataset("exchange_rate")
    else:
        raise NotImplementedError("Dataset %s not valid" % data_type)
    assert x_real.shape[0] == 1
    x_real = rolling_window(x_real[0], p + q)
    return pipeline, x_real  # ETTm2 -> x_real.shape [timesteps, p + q, N]
