...
from lib.data import download_man_ahl_dataset, download_mit_ecg_dataset, get_data
from lib.plot import create_summary, savefig
from lib.utils import load_pickle, pickle_it, sample_indices
...
def get_algo(algo_id, base_config, dataset, data_params, x_real):
    if algo_id == "SigCWGAN":
        ...
        algo = ALGOS[algo_id](
            x_real=x_real, config=algo_config, base_config=base_config
        )
    else:
...
def run_test(algo_id, base_config, base_dir, dataset, spec, slen, size, data_params={}):
    """Create the experiment directory, calibrate algorithm, store relevant parameters."""
    print("Executing: %s, %s, %s" % (algo_id, dataset, spec))

    # make dirs
    experiment_directory = pt.join(
        base_dir, dataset, spec, "seed={}".format(base_config.seed), algo_id
    )
    ...
    # obtain preprocessed train data
    ...
    with torch.no_grad():
        # size = int(x_real_train.shape[0]) ### same size as the training data
        batch_size = base_config.batch_size
        results = np.zeros(
            (size, slen, x_real_train.shape[2])
        )  ### np.zeros((size, 4000, x_real_train.shape[2]))
        ...
        while start < size:
            ...
            x_past = (
                x_real_train[indices, : base_config.p].clone().to(base_config.device)
            )
            x_fake_future = algo.G.sample(slen, x_past)  ### algo.G.sample(4000, x_past)

            if isinstance(x_fake_future, Tuple):
                x_fake = x_fake_future[0]
            else:
                x_fake = x_fake_future
            # x_fake = pipeline.inverse_transform(x_fake) ### for eval in rtsgan setting

            results[start : start + this_size :, :, :] = x_fake.cpu().numpy()
            start += this_size
            print(f"{start}/{size} = {round(start/size, 4)}")

        save_dir = os.path.join(experiment_directory, f"sl{args.slen}")
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        np.save(os.path.join(save_dir, "gen.npy"), results)
        print("complete generation")
        print("sample size: ", size)

        # calculate metrics
        acf = calculate_metrics(
            torch.from_numpy(results.astype(np.float32)).clone(), x_real_train[:size]
        )
        print(acf)
...
def run(algo_id, base_config, base_dir, dataset, spec, data_params={}):
    ...
    # obtain preprocessed train dataq
    pipeline, x_real = get_data(dataset, base_config.p, base_config.q, **data_params)
    x_real_train = x_real
    ...
    pickle_it(
        algo.G.to("cpu").state_dict(), pt.join(experiment_directory, "G_weights.torch")
    )
    pickle_it(
        algo.D.to("cpu").state_dict(), pt.join(experiment_directory, "D_weights.torch")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-slen", default=4000, type=int)  # for testing
    parser.add_argument("-sample_size", default=1000, type=int)  # for testing
